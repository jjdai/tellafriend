...
//define( '_MYMENU_CONSTANT_IN_MODINFO' , '_MI_TAF_MODNAME' ) ;
...
$prs = $xoopsDB->query( "SELECT l.lid, l.uid, l.ip, l.agent, l.mail_fromemail, l.mail_to, l.timestamp, u.uname, result, date_send"
                      . " FROM $log_table l LEFT JOIN ".$xoopsDB->prefix("users")." u ON l.uid=u.uid"
                      . " ORDER BY date_send DESC LIMIT $pos,$num" ) ;
...
while( list( $lid , $uid , $ip , $agent , $mail_fromemail , $mail_to , $timestamp , $uname, $result , $date_send) = $xoopsDB->fetchRow( $prs ) ) {
  ...
  <tr>
    ...
    <td class='$oddeven'>".date(_AM_TAF_FORMAT_DATE,strtotime($date_send))."</td>
...
echo "
  <tr>
    <td colspan='6' align='left'>"._AM_TAF_LABEL_REMOVE."<input type='button' value='"._AM_TAF_BUTTON_REMOVE."' onclick='if(confirm(\""._AM_TAF_JS_REMOVECONFIRM."\")){document.MainForm.action.value=\"delete\"; submit();}' /></td>
  </tr>
</table>
</form>
</td></tr></table>
